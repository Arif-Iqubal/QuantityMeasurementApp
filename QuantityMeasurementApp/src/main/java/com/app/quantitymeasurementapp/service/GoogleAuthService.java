...
public class GoogleAuthService {
	...
	public AuthResponse handleGoogleAuth(String code) {
		try {
			...
			/*
			 * This params have application Owner - google credentials [When user hit login
			 * with google then this params and header go to authorization server to get
			 * access token]
			 */
			...
			/*
			 * Here - we get the access token - with this access token server can get
			 * details of user
			 */
			String idToken = (String) tokenResponse.getBody().get("id_token");

			/*
			 * Here - this userInfor URL- help to get the user details - in this URL - we
			 * add the access token
			 */
			String userInfoUrl = "https://oauth2.googleapis.com/tokeninfo?id_token=" + idToken;

			/*
			 * Here - server again hit the get request to google server, but now they hit
			 * with access token, so now we get the entity details of user
			 */
			ResponseEntity<Map> userInfoResponse = restTemplate.getForEntity(userInfoUrl, Map.class);

			/*
			 * Every things well here - means status code 200 - means token correct,
			 * userInfoUrl correct, and we got userResponse
			 */
